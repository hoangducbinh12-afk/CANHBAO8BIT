...
# --- 1. CSS CHUẨN MOBILE ---
st.set_page_config(page_title="AI MATRIX ELITE V15.9.5", layout="wide")
st.markdown("""
    <style>
...
if st.session_state.data_list:
    ...
    if st.button("🚀 PHÂN TÍCH ELITE T1-T2"):
        # Chỉ xét nhịp T-1 và T-2
        n_t1 = num_soi
        ...
        f1 = get_f(n_t1); f2 = get_f(n_t2) if n_t2 is not None else Counter()
        ...
        if fol_bits:
            ...
            # --- SCORES CLEAN (CỐI & KẾT) ---
            scores_clean = {}
            for i in range(100):
                s = 0; i_bits = get_8bit(i)
                # 1. Khớp Bit
                s += sum([t_probs[j] if i_bits[j]==1 else (1-t_probs[j]) for j in range(8)]) * 100000
                # 2. Đa tầng trễ (T-1 ưu tiên, T-2 hỗ trợ nhẹ)
                s += f1.get(i, 0) * 600000  # Đẩy mạnh T-1 lên
                s += f2.get(i, 0) * 150000  # T-2 cộng điểm ít hơn bản cũ
                # 3. Thưởng Siêu Cối
        ...
        else: st.warning("Mẫu quét quá mỏng.")
    ...
    st.download_button("📥 TẢI DATA", exp_json, "Elite_Matrix_V15_9_5.json", "application/json")
