...
# --- 1. CONFIG & CSS ---
st.set_page_config(page_title="CANH BAO 8 BIT V10.7", layout="wide")
st.markdown("""
    <style>
...
with st.sidebar:
    ...
    if st.session_state.history:
        ...
        st.download_button("💾 LƯU MASTER", js, "master_v107.json")
...
if len(st.session_state.history) >= 10:
    ...
    tc1.markdown(f"<div class='tier-card tier-4'><b>Tầng 4/4 (Loại ngay):</b><br>{' '.join([x['S'] for x in scored if x['Tier']==4])}</div>", unsafe_allow_html=True)
    tc2.markdown(f"<div class='tier-card tier-3'><b>Tầng 3/4:</b><br>{' '.join([x['S'] for x in scored if x['Tier']==3])}</div>", unsafe_allow_html=True)
    tc3.markdown(f"<div class='tier-card tier-2'><b>Tầng 2/4:</b><br>{' '.join([x['S'] for x in scored if x['Tier']==2])}</div>", unsafe_allow_html=True)
    ...
    for h in sorted(st.session_state.history, key=lambda x: x['Kỳ'], reverse=True):
        ...
        # FIX KEYERROR: Dùng .get() để tránh lỗi nếu thiếu Rank
    ...
    st.warning("Nạp 10 kỳ để kích hoạt.")
